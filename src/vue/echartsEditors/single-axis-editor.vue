<template>
    <chart-section :title="t(`grapesjs-echarts-presets.config.singleAxis.label`)">
        <div class="gjs-trt-traits gjs-one-bg gjs-two-color">
            <ep-select :label="t('grapesjs-echarts-presets.config.axis.orient.label')" v-model="axis.orient">
                <ep-option v-for="direction in directions" :key="direction" :value="direction"
                           :label="t(`grapesjs-echarts-presets.dict.direction.${direction}`)"></ep-option>
            </ep-select>
            <ep-position-selector :t="t" use-unit :value="position" :title="t('grapesjs-echarts-presets.config.axis.position.label')"></ep-position-selector>
            <ep-size-selector :t="t" use-unit :value="size" :title="t('grapesjs-echarts-presets.config.axis.size.label')"></ep-size-selector>
            ...
            <ep-input v-if="axis.type === 'value'" :label="t('grapesjs-echarts-presets.config.axis.min.label')" v-model="axis.min"></ep-input>
            <ep-input v-if="axis.type === 'value'" :label="t('grapesjs-echarts-presets.config.axis.max.label')" v-model="axis.max"></ep-input>
...
<script>
  ...
  import EpSizeSelector from "../widgets/size-selector"
  import {SERIES_TYPES, DIRECTIONS} from "@/vue/utils/smallDict";
  import {TEXT_STYLE, LINE_STYLE, AREA_STYLE} from "@/vue/utils/templates";

  export default {
    name: "single-axis-editor",
    ...
    watch: {
      ...
      position: {
        handler (newValue) {
          Object.assign(this.axis, newValue)
        },
        deep:true
      },
      size: {
        handler (newValue) {
          [this.axis.width, this.axis.height] = newValue
        },
        deep:true
      }
    },
